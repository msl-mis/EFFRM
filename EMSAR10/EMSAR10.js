...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar10003 = $('#MasterPage_MasterPageContent_emsar10003_txt');
	if(temsar10003.length>0){
		var temsar10003Value = $('#MasterPage_MasterPageContent_emsar10003_txt').val().trim();
		if (temsar10003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR10', 'emsar10003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR10.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar10004_txt").value;         
    }
}