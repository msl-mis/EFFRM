...
namespace tw.com.dsc.easyflowDotNet.forms
{
	...
	/// ODMULLBCN01 的摘要描述。
	/// </summary>
	public partial class ODMULLBCN01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMULLBCN01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMULLBCN01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMULLBCN01", "HeadTabStrip01", tLanguageType);
			...
			this.label1.Text = MultiLanguage.GetComment("FD", "ODMULLBCN01", "label1", tLanguageType);
			this.open_idname.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "open_idname", tLanguageType);
			this.prd_category.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "prd_category", tLanguageType);
			this.factory.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "factory", tLanguageType);
			this.label_type.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "label_type", tLanguageType);
			this.class.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "class", tLanguageType);
			this.invoice_no.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "invoice_no", tLanguageType);
			this.invoice_date.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "invoice_date", tLanguageType);
			this.length.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "length", tLanguageType);
			this.quantity.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "quantity", tLanguageType);
			this.fees_yield.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "fees_yield", tLanguageType);
			this.fees_pcs.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "fees_pcs", tLanguageType);
			this.fees_1000pcs.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "fees_1000pcs", tLanguageType);
			this.fees_handling.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "fees_handling", tLanguageType);
			this.fees_service.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "fees_service", tLanguageType);
			this.note.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "note", tLanguageType);
			this.q1.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "q1", tLanguageType);
			this.r1.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "r1", tLanguageType);
			this.q2.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "q2", tLanguageType);
			this.r2.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "r2", tLanguageType);
			this.q3.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "q3", tLanguageType);
			this.r3.Title = MultiLanguage.GetComment("FD", "ODMULLBCN01", "r3", tLanguageType);
		...
		protected override void Page_Prender(object sender, EventArgs e)
		{
			...
			if(strToolTipParameter=="Y"){
				this.note.ToolTip = this.note.Text;
			...
			#region 自訂下拉選項
			#region 單頭頁籤Head01-prd_category下拉選項
			if(!IsPostBack)
			{
				this.prd_category.Items.Clear();//清空所有選項
				tSQL = @"select distinct product_category From ul_label"; 
				tReturnValue = createSQLSERVERDataTable(tSQL, "Persist Security Info=False;server=192.168.10.22; database=Price; User ID=sa; pwd=yzf;;Min Pool Size=0;Max Pool Size=500;Connect Timeout=600;", "product_category", "product_category");
				createSelectItemFromDB(tReturnValue, prd_category);
				if(MasterObj.NowRow != null)
				{
					prd_category.DDLInput.SelectedValue = (MasterObj.NowRow["prd_category"].Value == null ? "" : MasterObj.NowRow["prd_category"].Value.ToString());
				}

				//轉會表單
				if(base.CmdForwardApproveForm == "Y" && MasterObj.NewRow != null)
				{
					prd_category.DDLInput.SelectedValue = (MasterObj.NewRow["prd_category"].Value == null ? "" : MasterObj.NewRow["prd_category"].Value.ToString());
				}
			...
			#endregion 單頭頁籤Head01-prd_category下拉選項

			#region 單頭頁籤Head01-label_type下拉選項
			if(!IsPostBack)
			{
				this.label_type.Items.Clear();//清空所有選項
				tSQL = @"select distinct null From ul_label"; 
				tReturnValue = createSQLSERVERDataTable(tSQL, "Persist Security Info=False;server=192.168.10.22; database=Price; User ID=sa; pwd=yzf;;Min Pool Size=0;Max Pool Size=500;Connect Timeout=600;", "null", "null");
				createSelectItemFromDB(tReturnValue, label_type);
				if(MasterObj.NowRow != null)
				{
					label_type.DDLInput.SelectedValue = (MasterObj.NowRow["label_type"].Value == null ? "" : MasterObj.NowRow["label_type"].Value.ToString());
				}

				//轉會表單
				if(base.CmdForwardApproveForm == "Y" && MasterObj.NewRow != null)
				{
					label_type.DDLInput.SelectedValue = (MasterObj.NewRow["label_type"].Value == null ? "" : MasterObj.NewRow["label_type"].Value.ToString());
				}
			...
			#endregion 單頭頁籤Head01-label_type下拉選項

			#region 單頭頁籤Head01-class下拉選項
			if(!IsPostBack)
			{
				this.class.Items.Clear();//清空所有選項
				tSQL = @"select distinct null From ul_label"; 
				tReturnValue = createSQLSERVERDataTable(tSQL, "Persist Security Info=False;server=192.168.10.22; database=Price; User ID=sa; pwd=yzf;;Min Pool Size=0;Max Pool Size=500;Connect Timeout=600;", "null", "null");
				createSelectItemFromDB(tReturnValue, class);
				if(MasterObj.NowRow != null)
				{
					class.DDLInput.SelectedValue = (MasterObj.NowRow["class"].Value == null ? "" : MasterObj.NowRow["class"].Value.ToString());
				}

				//轉會表單
				if(base.CmdForwardApproveForm == "Y" && MasterObj.NewRow != null)
				{
					class.DDLInput.SelectedValue = (MasterObj.NewRow["class"].Value == null ? "" : MasterObj.NewRow["class"].Value.ToString());
				}
			...
			#endregion 單頭頁籤Head01-class下拉選項
			#endregion 自訂下拉選項
		...
		{
			// 作業代號
			this.TaskId = "ODMULLBCN01";
			// 表單代號
			this.formID = "ODMULLBCN01";
...
where resfc001=@resfc001 and resfc002=@resfc002 and resfc003=@resfc003 and ISNULL(resfc004,'')<>''";
		...
		protected override void settingClientFunction()
		{
			...
			//單頭控制項
			open_idname.HtmImg.Attributes.Add("onclick",MIMJUtil.getClickParams("MasterPage_MasterPageContent_open_idname_btn","MasterPage_MasterPageContent_open_idname_txt","S"));
			open_idname.TxtInput.Attributes.Add("onblur",MIMJUtil.getBlurParams("MasterPage_MasterPageContent_open_idname_txt","OpenMIMJ01_26_open_idname","MasterPage_MasterPageContent_open_idname_txt"));
			open_idname.TxtInput.Attributes.Add("onchange","AddtoHash('MasterPage_MasterPageContent_open_idname_txt');");
		...
		protected override void buildMiMjManager(Hashtable pMiMjManager)
		{
			...
			if ( FormStatus == EFFormStatus.CREATE ) {
				pMiMjManager.Add("open_idname_0", open_idname);
		...
		{
			defalutHash.Add("odmullbcn01001", this.formID);
			defalutHash.Add("odmullbcn01002", this.SheetNo);
		}
...
order by resdd003 desc";
		...
		{
			pReport = "ODMULLBCN01";
			pReportID = "ODMULLBCN01_02";//憑證式
			pWhere = "AND (odmullbcn01001='" + this.formID + "') AND (odmullbcn01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMULLBCN01";
