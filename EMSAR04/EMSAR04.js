...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar04003 = $('#MasterPage_MasterPageContent_emsar04003_txt');
	if(temsar04003.length>0){
		var temsar04003Value = $('#MasterPage_MasterPageContent_emsar04003_txt').val().trim();
		if (temsar04003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR04', 'emsar04003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR04.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar04004_txt").value;
