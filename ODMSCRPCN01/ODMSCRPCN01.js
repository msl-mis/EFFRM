...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1").length>0){
		...
		//數值驗証
		var tassprice = $('#MasterPage_MasterPageContent_assprice_txt');
		if(tassprice.length>0){
			if('readonly' !== tassprice.attr('readonly') &&
				'disabled' !== tassprice.attr('disabled')){
				var tasspriceValue = $('#MasterPage_MasterPageContent_assprice_txt').val().trim();
				if(tasspriceValue.length==0 || isNaN(tasspriceValue))
				{
					tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
				}
				else
				{
					if (tasspriceValue < 1 || tasspriceValue > 10000000)
					{
						tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
					}
				}
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2")[0].checked){
		//數值驗証
		var toutprice = $('#MasterPage_MasterPageContent_outprice_txt');
		if(toutprice.length>0){
			if('readonly' !== toutprice.attr('readonly') &&
				'disabled' !== toutprice.attr('disabled')){
				var toutpriceValue = $('#MasterPage_MasterPageContent_outprice_txt').val().trim();
				if(toutpriceValue.length==0 || isNaN(toutpriceValue))
				{
					tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
				}
				else
				{
					if (toutpriceValue < 1 || toutpriceValue > 100000000000)
					{
						tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
					}
				}
	...
	if(tassprice.length>0){
		var tasspriceValue = $('#MasterPage_MasterPageContent_assprice_txt').val().trim();
		if (tasspriceValue.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tasspriceValue)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	//不允許空白驗證
	var tmaterialname = $('#MasterPage_MasterPageContent_materialname_txt');
	if(tmaterialname.length>0){
		var tmaterialnameValue = $('#MasterPage_MasterPageContent_materialname_txt').val().trim();
		if (tmaterialnameValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'materialname', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(toutprice.length>0){
		var toutpriceValue = $('#MasterPage_MasterPageContent_outprice_txt').val().trim();
		if (toutpriceValue.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(toutpriceValue)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tStatus=="CREATE" || tStatus=="DISPLAY" || tStatus==""){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1").length>0){
			if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1")[0].checked){
				...
				$("#MasterPage_MasterPageContent_assprice").hide();
			}
		}

		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2").length>0){
			if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2")[0].checked){
				$("#MasterPage_MasterPageContent_outprice").show();
			}
			else{
				$("#MasterPage_MasterPageContent_outprice").hide();
			}
		}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMSCRPCN01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMSCRPCN01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+='CN報廢申請單-'+
$('#MasterPage_MasterPageContent_materialname_txt').val();
...
function chkTriggerFieldNull_Head()
{
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1")[0].checked){
			if($("#MasterPage_MasterPageContent_assprice_txt").val().trim().length==0){
				tErr+='[assprice-'+getI18NForSpecial('FD', 'ODMSCRPCN01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#MasterPage_MasterPageContent_assprice_txt").val('');
		}
	}

	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2")[0].checked){
			if($("#MasterPage_MasterPageContent_outprice_txt").val().trim().length==0){
				tErr+='[outprice-'+getI18NForSpecial('FD', 'ODMSCRPCN01', 'outprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#MasterPage_MasterPageContent_outprice_txt").val('');
		}
	}
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMSCRPCN01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
