...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar18003 = $('#MasterPage_MasterPageContent_emsar18003_txt');
	if(temsar18003.length>0){
		var temsar18003Value = $('#MasterPage_MasterPageContent_emsar18003_txt').val().trim();
		if (temsar18003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR18', 'emsar18003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR18.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar18004_txt").value;
