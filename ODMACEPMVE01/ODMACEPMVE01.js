...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if (tStatus == "CREATE")
	{
		...
		if(topendetp.length>0){
			if('readonly' !== topendetp.attr('readonly') &&
				'disabled' !== topendetp.attr('disabled')){
				var topendetpValue = $('#MasterPage_MasterPageContent_opendetp_txt').val();
				if (topendetpValue.length < 1 || topendetpValue.length > 50)
				{
					tErr += '「' + getI18NForSpecial('FD', 'ODMACEPMVE01', 'opendetp', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMACEPMVE01', 'opendetp_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '\r\n';
				}
			}
		}
		...
		//Radio Button 驗証
			...
			{
				tErr += getI18NForSpecial('FD', 'ODMACEPMVE01', 'kind_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
			}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMACEPMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMACEPMVE01.ajaxGetMoneyWord(tControlNumValue).value;
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMACEPMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
