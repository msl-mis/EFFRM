...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if(ttxtamt01.length>0){
		...
		if (ttxtamt01Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(ttxtamt01.length>0){
		var ttxtamt01Value = $('#MasterPage_MasterPageContent_txtamt01_txt').val().trim();
		if (ttxtamt01Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt01Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt02.length>0){
		...
		if (ttxtamt02Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt02Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt03.length>0){
		...
		if (ttxtamt03Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt03Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//Int格式驗証
	var ttxtamt04 = $('#MasterPage_MasterPageContent_txtamt04_txt');
	if(ttxtamt04.length>0){
		var ttxtamt04Value = $('#MasterPage_MasterPageContent_txtamt04_txt').val().trim();
		if (ttxtamt04Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt04Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt04', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//Int格式驗証
	var ttxtamt05 = $('#MasterPage_MasterPageContent_txtamt05_txt');
	if(ttxtamt05.length>0){
		var ttxtamt05Value = $('#MasterPage_MasterPageContent_txtamt05_txt').val().trim();
		if (ttxtamt05Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt05Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt05', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//Int格式驗証
	var ttxtamt06 = $('#MasterPage_MasterPageContent_txtamt06_txt');
	if(ttxtamt06.length>0){
		var ttxtamt06Value = $('#MasterPage_MasterPageContent_txtamt06_txt').val().trim();
		if (ttxtamt06Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt06Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt06', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//Int格式驗証
	var ttxtamt07 = $('#MasterPage_MasterPageContent_txtamt07_txt');
	if(ttxtamt07.length>0){
		var ttxtamt07Value = $('#MasterPage_MasterPageContent_txtamt07_txt').val().trim();
		if (ttxtamt07Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt07Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt07', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//Int格式驗証
	var ttxtamt08 = $('#MasterPage_MasterPageContent_txtamt08_txt');
	if(ttxtamt08.length>0){
		var ttxtamt08Value = $('#MasterPage_MasterPageContent_txtamt08_txt').val().trim();
		if (ttxtamt08Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt08Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtamt08', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}

	//不允許空白驗證
	var ttxtname01 = $('#MasterPage_MasterPageContent_txtname01_txt');
	if(ttxtname01.length>0){
		var ttxtname01Value = $('#MasterPage_MasterPageContent_txtname01_txt').val().trim();
		if (ttxtname01Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtname01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}

	//float格式驗証
	var ttxtprice01 = $('#MasterPage_MasterPageContent_txtprice01_txt');
	if(ttxtprice01.length>0){
		var ttxtprice01Value = $('#MasterPage_MasterPageContent_txtprice01_txt').val().trim();
		if (ttxtprice01Value.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(ttxtprice01Value)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtprice01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
				ttxtprice01Value=ttxtprice01Value.replace(/\,/g,'');
				$('#MasterPage_MasterPageContent_txtprice01_txt').val(parseFloat(ttxtprice01Value).toFixed(2));
			}
		}
	}

	//float格式驗証
	var ttxtprice02 = $('#MasterPage_MasterPageContent_txtprice02_txt');
	if(ttxtprice02.length>0){
		var ttxtprice02Value = $('#MasterPage_MasterPageContent_txtprice02_txt').val().trim();
		if (ttxtprice02Value.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(ttxtprice02Value)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYCN01', 'txtprice02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
				ttxtprice02Value=ttxtprice02Value.replace(/\,/g,'');
				$('#MasterPage_MasterPageContent_txtprice02_txt').val(parseFloat(ttxtprice02Value).toFixed(2));
			}
		}
	}

		//Radio Button 驗証
		if ($('#MasterPage_MasterPageContent_rddept_txt').length>0 && document.getElementById('MasterPage_MasterPageContent_rddept_txt').value == '')
		{
			tErr += getI18NForSpecial('FD', 'ODMAPYCN01', 'rddept_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		}
		//Radio Button 驗証
		...
		{
			tErr += getI18NForSpecial('FD', 'ODMAPYCN01', 'rditem_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		}
...
function InitTriggerMust(tStatus){
	if(tStatus=="CREATE" || tStatus=="DISPLAY" || tStatus==""){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3").length>0){
			if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3")[0].checked){
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMAPYCN01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMAPYCN01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+='CN新增申請單-'+
$('#MasterPage_MasterPageContent_applydate_txt').val()+'-';
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3")[0].checked){
			if($("#MasterPage_MasterPageContent_txtother_txt").val().trim().length==0){
				tErr+='[txtother-'+getI18NForSpecial('FD', 'ODMAPYCN01', 'txtother', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMAPYCN01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
