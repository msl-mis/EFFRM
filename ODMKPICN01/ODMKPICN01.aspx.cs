...
namespace tw.com.dsc.easyflowDotNet.forms
{
	...
	/// ODMKPICN01 的摘要描述。
	/// </summary>
	public partial class ODMKPICN01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMKPICN01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMKPICN01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMKPICN01", "HeadTabStrip01", tLanguageType);
			...
			this.txtuser.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "txtuser", tLanguageType);
			this.date.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "date", tLanguageType);
			this.txtname01.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "txtname01", tLanguageType);
			this.report.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "report", tLanguageType);
			this.queryemp.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "queryemp", tLanguageType);
			this.superid.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "superid", tLanguageType);
			this.id.Title = MultiLanguage.GetComment("FD", "ODMKPICN01", "id", tLanguageType);
		...
		protected override void Page_Prender(object sender, EventArgs e)
		{
			...
			if(strToolTipParameter=="Y"){
				this.report.ToolTip = this.report.Text;
		...
		{
			// 作業代號
			this.TaskId = "ODMKPICN01";
			// 表單代號
			this.formID = "ODMKPICN01";
...
where resfc001=@resfc001 and resfc002=@resfc002 and resfc003=@resfc003 and ISNULL(resfc004,'')<>''";
		...
		{
			defalutHash.Add("odmkpicn01001", this.formID);
			defalutHash.Add("odmkpicn01002", this.SheetNo);
			defalutHash.Add("date", DateTime.Now.ToString("yyyyMMdd"));
			defalutHash.Add("id", this.UserInfo.EmployeeId.ToString());
...
order by resdd003 desc";
		...
		protected override void ReGetCondValue(object[,] pAryCondValue)
		{
			...
			tValue="";
			try{
				tValue = (this.queryemp.Value.Trim());
			}
			catch(Exception e){
				tValue="";
			}
			pAryCondValue[0, 0] = "sign";
			...
			tValue="";
			try{
				tValue = this.UserInfo.EmployeeId.ToString().Trim();
			}
			catch(Exception e){
				tValue="";
			}
			pAryCondValue[0, 2] = "id";
		...
		{
			pReport = "ODMKPICN01";
			pReportID = "ODMKPICN01_02";//憑證式
			pWhere = "AND (odmkpicn01001='" + this.formID + "') AND (odmkpicn01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMKPICN01";
