...
{
	/// <summary>
	/// ODMPURPAY01_TEST 的摘要描述。
	/// </summary>
	public partial class ODMPURPAY01_TEST : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMPURPAY01_TEST));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMPURPAY01_TEST","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "HeadTabStrip01", tLanguageType);
			...
			this.txtuser.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "txtuser", tLanguageType);
			this.useday.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "useday", tLanguageType);
			this.otherpay.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "otherpay", tLanguageType);
			this.paydate.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "paydate", tLanguageType);
			this.querypur.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "querypur", tLanguageType);
			this.na003.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "na003", tLanguageType);
			this.ma002.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "ma002", tLanguageType);
			this.tc005.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "tc005", tLanguageType);
			this.totalamt.Title = MultiLanguage.GetComment("FD", "ODMPURPAY01_TEST", "totalamt", tLanguageType);
		...
		{
			// 作業代號
			this.TaskId = "ODMPURPAY01_TEST";
			// 表單代號
			this.formID = "ODMPURPAY01_TEST";
...
where resfc001=@resfc001 and resfc002=@resfc002 and resfc003=@resfc003 and ISNULL(resfc004,'')<>''";
		...
		protected override void settingClientFunction()
		{
			...
			//單頭控制項
			querypur.HtmImg.Attributes.Add("onclick",MIMJUtil.getClickParams("MasterPage_MasterPageContent_querypur_btn","MasterPage_MasterPageContent_querypur_txt","S"));
			querypur.TxtInput.Attributes.Add("onblur",MIMJUtil.getBlurParams("MasterPage_MasterPageContent_querypur_txt","OpenMIMJ01_05_querypur","MasterPage_MasterPageContent_querypur_txt"));
			querypur.TxtInput.Attributes.Add("onchange","AddtoHash('MasterPage_MasterPageContent_querypur_txt');");
...
this.pay_ctrolRadio2.Attributes.Add("onclick", "InitTriggerMust('" + base.FormStatus.ToString() + "');document.getElementById('MasterPage_MasterPageContent_pay_txt').value = '2';");
		...
		protected override void buildMiMjManager(Hashtable pMiMjManager)
		{
			...
			if ( FormStatus == EFFormStatus.CREATE ) {
				pMiMjManager.Add("querypur_0", querypur);
		...
		{
			defalutHash.Add("odmpurpay_test001", this.formID);
			defalutHash.Add("odmpurpay_test002", this.SheetNo);
...
order by resdd003 desc";
		...
		{
			pReport = "ODMPURPAY01_TEST";
			pReportID = "ODMPURPAY01_TEST_02";//憑證式
			pWhere = "AND (odmpurpay_test001='" + this.formID + "') AND (odmpurpay_test002='" + this.SheetNo + "')";
			pReportDirectory = "ODMPURPAY01_TEST";
