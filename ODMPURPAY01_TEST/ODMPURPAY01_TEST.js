	...
	if(tStatus=="CREATE" || tStatus=="DISPLAY" || tStatus==""){
		if($("#MasterPage_MasterPageContent_pay_ctrolRadio2").length>0){
			if($("#MasterPage_MasterPageContent_pay_ctrolRadio2")[0].checked){
				$("#MasterPage_MasterPageContent_otherpay").show();
			}
			else{
				$("#MasterPage_MasterPageContent_otherpay").hide();
			}
		}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMPURPAY01_TEST.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMPURPAY01_TEST.ajaxGetMoneyWord(tControlNumValue).value;
...
function chkTriggerFieldNull_Head()
{
	...
	if($("#MasterPage_MasterPageContent_pay_ctrolRadio2").length>0){
		if($("#MasterPage_MasterPageContent_pay_ctrolRadio2")[0].checked){
			if($("#MasterPage_MasterPageContent_otherpay_txt").val().trim().length==0){
				tErr+='[otherpay-'+getI18NForSpecial('FD', 'ODMPURPAY01_TEST', 'otherpay', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#MasterPage_MasterPageContent_otherpay_txt").val('');
		}
	}
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMPURPAY01_TEST.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
