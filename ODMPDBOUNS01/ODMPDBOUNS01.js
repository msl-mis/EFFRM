	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMPDBOUNS01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMPDBOUNS01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+=
$('#MasterPage_MasterPageContent_txtdept_txt').val()+' -';
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMPDBOUNS01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
