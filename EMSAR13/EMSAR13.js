...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar13003 = $('#MasterPage_MasterPageContent_emsar13003_txt');
	if(temsar13003.length>0){
		var temsar13003Value = $('#MasterPage_MasterPageContent_emsar13003_txt').val().trim();
		if (temsar13003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR13', 'emsar13003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR13.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar13004_txt").value;         
    }
}