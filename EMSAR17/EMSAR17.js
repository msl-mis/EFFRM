...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar17003 = $('#MasterPage_MasterPageContent_emsar17003_txt');
	if(temsar17003.length>0){
		var temsar17003Value = $('#MasterPage_MasterPageContent_emsar17003_txt').val().trim();
		if (temsar17003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR17', 'emsar17003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR17.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar17004_txt").value;
