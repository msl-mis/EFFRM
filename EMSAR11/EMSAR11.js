...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar11003 = $('#MasterPage_MasterPageContent_emsar11003_txt');
	if(temsar11003.length>0){
		var temsar11003Value = $('#MasterPage_MasterPageContent_emsar11003_txt').val().trim();
		if (temsar11003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR11', 'emsar11003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR11.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar11004_txt").value;         
    }
}