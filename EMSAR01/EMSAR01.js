...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar01003 = $('#MasterPage_MasterPageContent_emsar01003_txt');
	if(temsar01003.length>0){
		var temsar01003Value = $('#MasterPage_MasterPageContent_emsar01003_txt').val().trim();
		if (temsar01003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR01', 'emsar01003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR01.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar01004_txt").value;
