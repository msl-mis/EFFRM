...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if(ttxtamt01.length>0){
		...
		if (ttxtamt01Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(ttxtamt01.length>0){
		...
		if (ttxtamt01Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt01Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt02.length>0){
		...
		if (ttxtamt02Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt02Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt03.length>0){
		...
		if (ttxtamt03Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt03Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt04.length>0){
		...
		if (ttxtamt04Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt04Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt04', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt05.length>0){
		...
		if (ttxtamt05Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt05Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt05', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt06.length>0){
		...
		if (ttxtamt06Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt06Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt06', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt07.length>0){
		...
		if (ttxtamt07Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt07Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt07', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtamt08.length>0){
		...
		if (ttxtamt08Value.length>0){
			if(!/^-?\d+$/.test(ttxtamt08Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtamt08', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(ttxtname01.length>0){
		...
		if (ttxtname01Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtname01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
		...
		{
			tErr += getI18NForSpecial('FD', 'ODMAPYMVE01', 'rddept_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		}
		//Radio Button 驗証
		...
		{
			tErr += getI18NForSpecial('FD', 'ODMAPYMVE01', 'rditem_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMAPYMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMAPYMVE01.ajaxGetMoneyWord(tControlNumValue).value;
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3")[0].checked){
			if($("#MasterPage_MasterPageContent_txtother_txt").val().trim().length==0){
				tErr+='[txtother-'+getI18NForSpecial('FD', 'ODMAPYMVE01', 'txtother', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMAPYMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
