...
{
	/// <summary>
	/// ODMSYSMDFMVE01 的摘要描述。
	/// </summary>
	public partial class ODMSYSMDFMVE01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMSYSMDFMVE01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMSYSMDFMVE01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "HeadTabStrip01", tLanguageType);
			...
			this.texdept.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "texdept", tLanguageType);
			this.datetime1.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "datetime1", tLanguageType);
			this.datetime2.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "datetime2", tLanguageType);
			this.sysname.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "sysname", tLanguageType);
			this.pdname.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "pdname", tLanguageType);
			this.content.Title = MultiLanguage.GetComment("FD", "ODMSYSMDFMVE01", "content", tLanguageType);
		...
		protected override void Page_Prender(object sender, EventArgs e)
		{
			...
			if(strToolTipParameter=="Y"){
				this.sysname.ToolTip = this.sysname.Text;
this.pdname.ToolTip = this.pdname.Text;
this.content.ToolTip = this.content.Text;
		...
		{
			// 作業代號
			this.TaskId = "ODMSYSMDFMVE01";
			// 表單代號
			this.formID = "ODMSYSMDFMVE01";
...
this.rditem_ctrolRadio1.Attributes.Add("onclick", "document.getElementById('MasterPage_MasterPageContent_rditem_txt').value = '1';");
		...
		{
			defalutHash.Add("odmsysmdfmve01001", this.formID);
			defalutHash.Add("odmsysmdfmve01002", this.SheetNo);
			defalutHash.Add("datetime1", DateTime.Now.ToString("yyyyMMdd"));
			defalutHash.Add("datetime2", DateTime.Now.ToString("yyyyMMdd"));
			defalutHash.Add("texdept", ajaxGetDepartmentName());
		}
...
order by resdd003 desc";
		...
		{
			pReport = "ODMSYSMDFMVE01";
			pReportID = "ODMSYSMDFMVE01_02";//憑證式
			pWhere = "AND (odmsysmdfmve01001='" + this.formID + "') AND (odmsysmdfmve01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMSYSMDFMVE01";
