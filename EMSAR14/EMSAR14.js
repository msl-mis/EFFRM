...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar14003 = $('#MasterPage_MasterPageContent_emsar14003_txt');
	if(temsar14003.length>0){
		var temsar14003Value = $('#MasterPage_MasterPageContent_emsar14003_txt').val().trim();
		if (temsar14003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR14', 'emsar14003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR14.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar14004_txt").value;
