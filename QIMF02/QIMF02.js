...
var gField_companyid = "qimf02003";//公司別代號的id
//單頭自訂驗證
...
//2013/02/04:3.5.1.48:hiro:S00-20120626008:雙檔表單簽核鈕(發單、准、同意、會辦)增加單身驗證功能↑
{
	...
	//填表及簽核都要驗證
	//不允許空白驗證
	var tqimf02005 = $('#MasterPage_MasterPageContent_qimf02005_txt');
	if(tqimf02005.length>0){
		var tqimf02005Value = $('#MasterPage_MasterPageContent_qimf02005_txt').val().trim();
		if (tqimf02005Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'QIMF02', 'qimf02005', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}	

    //^_^20190828 edit by 06475 lijyun 將數值三位撇取消↓
	var qimf02010 = $('#MasterPage_MasterPageContent_qimf02010_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02010_txt').val(parseFloat(qimf02010));
	var qimf02017 = $('#MasterPage_MasterPageContent_qimf02017_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02017_txt').val(parseFloat(qimf02017));
	var qimf02018 = $('#MasterPage_MasterPageContent_qimf02018_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02018_txt').val(parseFloat(qimf02018));
	var qimf02019 = $('#MasterPage_MasterPageContent_qimf02019_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02019_txt').val(parseFloat(qimf02019));
	var qimf02020 = $('#MasterPage_MasterPageContent_qimf02020_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02020_txt').val(parseFloat(qimf02020));
	var qimf02029 = $('#MasterPage_MasterPageContent_qimf02029_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf02029_txt').val(parseFloat(qimf02029));
    //^_^20190828 edit by 06475 lijyun 將數值三位撇取消↑
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.QIMF02.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.QIMF02.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf = '';
	var tTitle = getI18NForSpecial('FD', 'QIMF02', 'HeadTabStrip01', '../../../src/_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢查記錄表
    //^_^ 20190822 edit by 06475 lijyun DSCBV0972A_38_0001↓
	tSubjectTxt += $("#MasterPage_MasterPageContent_qimf02005_txt").val() + "(" + $("#MasterPage_MasterPageContent_qimf02009_txt2hdn").val() + ")";
    //^_^ 20190822 edit by 06475 lijyun DSCBV0972A_38_0001↑
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.QIMF02.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
...
function qimf02006_onblur() {
    var UniID = document.getElementById("MasterPage_universalID").value;
    var qimf02024 = document.getElementById("MasterPage_MasterPageContent_qimf02024_txt"); //圖面一
    var qimf02025 = document.getElementById("MasterPage_MasterPageContent_qimf02025_txt"); //圖面二
    var qimf02026 = document.getElementById("MasterPage_MasterPageContent_qimf02026_txt"); //圖面三
    var qimf02006 = document.getElementById("MasterPage_MasterPageContent_qimf02006_txt"); //品號
    var qimf02015 = document.getElementById("MasterPage_MasterPageContent_qimf02015_txt"); //品號
    var companyid = document.getElementById("MasterPage_MasterPageContent_" + gField_companyid + "_ddl"); //品號

    //圖面一
    if (qimf02024 != null && qimf02024.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF02.LoadQIMF02IMG(qimf02024.value, "Drawing1", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02024").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf02024").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02024").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf02024").style.display = "none";
    ...
    //圖面二
    if (qimf02025 != null && qimf02025.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF02.LoadQIMF02IMG(qimf02025.value, "Drawing2", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02025").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf02025").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02025").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf02025").style.display = "none";
    ...
    //圖面三
    if (qimf02026 != null && qimf02026.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF02.LoadQIMF02IMG(qimf02026.value, "Drawing3", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02026").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf02026").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf02026").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf02026").style.display = "none";
    ...
    //抽樣計畫表
    if (companyid != null && companyid.value != "" && qimf02006 != null && qimf02006.value != ""
        && qimf02015 != null && qimf02015.value!="") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF02.LoadSamplingPlanIMG(companyid.value, qimf02006.value, qimf02015.value, UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
...
function qimf02005_onblur() {
    document.getElementById("MasterPage_MasterPageContent_qimf02006_txt").onchange();
    document.getElementById("MasterPage_MasterPageContent_qimf02006_txt").onblur();
}
...
function LoadJSONObject(tStatus) {
    var qimf02001 = document.getElementById("MasterPage_MasterPageContent_qimf02001_txt");//表單代號
    var qimf02002 = document.getElementById("MasterPage_MasterPageContent_qimf02002_txt");//表單單號
    var qimf02006 = document.getElementById("MasterPage_MasterPageContent_qimf02006_txt");//產品品號
    var qimf02015 = document.getElementById("MasterPage_MasterPageContent_qimf02015_txt");//檢驗版本
    var qimf02017 = document.getElementById("MasterPage_MasterPageContent_qimf02017_txt");//抽樣數
    var companyid = document.getElementById("MasterPage_MasterPageContent_" + gField_companyid + "_ddl"); //品號
    
    if (qimf02006 != null && qimf02006.value != "" && qimf02015 != null && qimf02015.value != "") {
        //先判斷是否有未完成的檢驗資料，取得檢驗資料
        obj_return = tw.com.dsc.easyflowDotNet.forms.QIMF02.ajaxLoadJSON([qimf02006.value, qimf02015.value, qimf02017.value, companyid.value, qimf02001.value, qimf02002.value]);
        if (obj_return.value != "") {
...
function DarwingCheckData(tStatus) {
    var tqimf02d003 = getI18NForSpecial('FD', 'QIMF02_d01', 'NO', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗項目序號
    var tqimf02d004 = getI18NForSpecial('FD', 'QIMF02_d01', 'qimf02d003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗項目
    var tqimf02d005 = getI18NForSpecial('FD', 'QIMF02_d01', 'qimf02d004', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗說明
	var tqimf02d006 = getI18NForSpecial('FD', 'QIMF02_d01', 'qimf02d006', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗結果
	var tqimf02d010 = getI18NForSpecial('FD', 'QIMF02_d01', 'qimf02d010', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//Min 
	var tqimf02d011 = getI18NForSpecial('FD', 'QIMF02_d01', 'qimf02d011', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//Max
    var thint_img = "../../_Common/AppUtil/Themes/images/Form/imgImportant.gif";

    var qimf02006 = document.getElementById("MasterPage_MasterPageContent_qimf02006_txt");//料號
    var qimf02015 = document.getElementById("MasterPage_MasterPageContent_qimf02015_txt");//檢驗版本
    var qimf02017 = document.getElementById("MasterPage_MasterPageContent_qimf02017_txt");//抽樣數
    var icount = parseInt(qimf02017.value);
    ...
    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf02d003).css("height", "63px"); row.append(cell);
    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").addClass("OEMTableWidth").text(tqimf02d004); row.append(cell);

    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf02d005).addClass("OEMTableWidth").addClass("OEMTableWidth");
    row.append(cell);

	//^_^ 20201219 edit by 13768 yating DSCBV0972A_38_0004 ↓
	cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf02d010).addClass("OEMTableWidth").addClass("OEMTableWidth");
	row.append(cell);

	cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf02d011).addClass("OEMTableWidth").addClass("OEMTableWidth");
	row.append(cell);
	//^_^ 20201219 edit by 13768 yating DSCBV0972A_38_0004 ↑

    cell = $("<td>").attr("colspan", icount).addClass("OEMFormFont").text(tqimf02d006); row.append(cell);
    row.append(cell);
    table.append(row);
    row = $("<tr>").addClass("GridviewScrollHeader");
