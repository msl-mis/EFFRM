...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temvar01003 = $('#MasterPage_MasterPageContent_emvar01003_txt');
	if(temvar01003.length>0){
		var temvar01003Value = $('#MasterPage_MasterPageContent_emvar01003_txt').val().trim();
		if (temvar01003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMVAR01', 'emvar01003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMVAR01.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emvar01004_txt").value;
