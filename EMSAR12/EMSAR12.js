...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar12003 = $('#MasterPage_MasterPageContent_emsar12003_txt');
	if(temsar12003.length>0){
		var temsar12003Value = $('#MasterPage_MasterPageContent_emsar12003_txt').val().trim();
		if (temsar12003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR12', 'emsar12003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR12.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar12004_txt").value;         
    }
}