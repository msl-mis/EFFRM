...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar05003 = $('#MasterPage_MasterPageContent_emsar05003_txt');
	if(temsar05003.length>0){
		var temsar05003Value = $('#MasterPage_MasterPageContent_emsar05003_txt').val().trim();
		if (temsar05003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR05', 'emsar05003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR05.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar05004_txt").value;         
    }
}