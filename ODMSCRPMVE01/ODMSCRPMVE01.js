...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if(tassprice.length>0){
		...
		if (tasspriceValue.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tasspriceValue)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPMVE01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tmaterialname.length>0){
		...
		if (tmaterialnameValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMSCRPMVE01', 'materialname', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMSCRPMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMSCRPMVE01.ajaxGetMoneyWord(tControlNumValue).value;
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio1")[0].checked){
			if($("#MasterPage_MasterPageContent_assprice_txt").val().trim().length==0){
				tErr+='[assprice-'+getI18NForSpecial('FD', 'ODMSCRPMVE01', 'assprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio2")[0].checked){
			if($("#MasterPage_MasterPageContent_outprice_txt").val().trim().length==0){
				tErr+='[outprice-'+getI18NForSpecial('FD', 'ODMSCRPMVE01', 'outprice', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMSCRPMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
