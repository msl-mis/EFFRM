...
namespace tw.com.dsc.easyflowDotNet.forms
{
	...
	/// ODMSCRPMVE01 的摘要描述。
	/// </summary>
	public partial class ODMSCRPMVE01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMSCRPMVE01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMSCRPMVE01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "HeadTabStrip01", tLanguageType);
			...
			this.label1.Text = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "label1", tLanguageType);
			this.label2.Text = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "label2", tLanguageType);
			this.usedate.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "usedate", tLanguageType);
			this.dept.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "dept", tLanguageType);
			this.username.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "username", tLanguageType);
			this.buydate.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "buydate", tLanguageType);
			this.materialsno.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "materialsno", tLanguageType);
			this.materialname.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "materialname", tLanguageType);
			this.location.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "location", tLanguageType);
			this.materialspec.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "materialspec", tLanguageType);
			this.scrpqty.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "scrpqty", tLanguageType);
			this.assprice.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "assprice", tLanguageType);
			this.scrpreason.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "scrpreason", tLanguageType);
			this.userid.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "userid", tLanguageType);
			this.superid.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "superid", tLanguageType);
			this.outprice.Title = MultiLanguage.GetComment("FD", "ODMSCRPMVE01", "outprice", tLanguageType);
		...
		protected override void Page_Prender(object sender, EventArgs e)
		{
			...
			if(strToolTipParameter=="Y"){
				this.materialspec.ToolTip = this.materialspec.Text;
this.scrpreason.ToolTip = this.scrpreason.Text;
			...
			rdtrans.Attributes["style"] = "display:none;";
			rdtrans_ctrolRadio0.Checked = (rdtrans.Value == "0"); rdtrans_ctrolRadio0.Enabled = rdtrans.InputEnabled;
			rdtrans_ctrolRadio1.Checked = (rdtrans.Value == "1"); rdtrans_ctrolRadio1.Enabled = rdtrans.InputEnabled;
		...
		{
			// 作業代號
			this.TaskId = "ODMSCRPMVE01";
			// 表單代號
			this.formID = "ODMSCRPMVE01";
...
this.rdtrans_ctrolRadio0.Attributes.Add("onclick", "document.getElementById('MasterPage_MasterPageContent_rdtrans_txt').value = '0';");
this.rdtrans_ctrolRadio1.Attributes.Add("onclick", "document.getElementById('MasterPage_MasterPageContent_rdtrans_txt').value = '1';");
		...
		{
			defalutHash.Add("rdtrans", "1");
			defalutHash.Add("odmscrpmve01001", this.formID);
			defalutHash.Add("odmscrpmve01002", this.SheetNo);
			...
			defalutHash.Add("userid", this.UserInfo.EmployeeId.ToString());
...
order by resdd003 desc";
		...
		protected override void ReGetCondValue(object[,] pAryCondValue)
		{
			...
			tValue="";
			try{
				tValue = (this.userid.Value.Trim());
			}
			catch(Exception e){
				tValue="";
			}
			pAryCondValue[0, 1] = "userID";
			...
			tDbl=0;
			try{
				tDbl = double.Parse(this.assprice.Value.Trim());
			}
			catch(Exception e){
				tDbl=0;
			}
			pAryCondValue[0, 3] = "assprice";
			...
			tDbl=0;
			try{
				tDbl = double.Parse(this.outprice.Value.Trim());
			}
			catch(Exception e){
				tDbl=0;
			}
			pAryCondValue[0, 4] = "outprice";
			...
			tDbl=0;
			try{
				tDbl = double.Parse(this.rdtrans.Value.Trim());
			}
			catch(Exception e){
				tDbl=0;
			}
			pAryCondValue[0, 5] = "rdtrans";
			...
			tValue="";
			try{
				tTemp = this.UserInfo.EmployeeId.ToString().Trim();
tTemp = objRE.FindEmplDeptID(tTemp, Session["strProcID"].ToString());
tValue = objRE.FindDeptName(tTemp, Session["strProcID"].ToString());
			}
			catch(Exception e){
				tValue="";
			}
			pAryCondValue[0, 6] = "dept";
		...
		{
			pReport = "ODMSCRPMVE01";
			pReportID = "ODMSCRPMVE01_02";//憑證式
			pWhere = "AND (odmscrpmve01001='" + this.formID + "') AND (odmscrpmve01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMSCRPMVE01";
