...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if(ttxtcontact.length>0){
		...
		if (ttxtcontactValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMDEBCN01', 'txtcontact', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMDEBCN01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMDEBCN01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+='CN水電扣款明細表-'+
$('#MasterPage_MasterPageContent_txtcontact_txt').val();
	...
	if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3").length>0){
		if($("#MasterPage_MasterPageContent_rditem_ctrolRadio3")[0].checked){
			if($("#MasterPage_MasterPageContent_txtother_txt").val().trim().length==0){
				tErr+='[txtother-'+getI18NForSpecial('FD', 'ODMDEBCN01', 'txtother', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMDEBCN01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
