...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var tiamar02003 = $('#MasterPage_MasterPageContent_iamar02003_txt');
	if(tiamar02003.length>0){
		var tiamar02003Value = $('#MasterPage_MasterPageContent_iamar02003_txt').val().trim();
		if (tiamar02003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'IAMAR02', 'iamar02003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.IAMAR02.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_iamar02004_txt").value;
