...
{
	/// <summary>
	/// ODMTESTV02_MIMJ 的摘要描述。
	/// </summary>
	public partial class ODMTESTV02_MIMJ : tw.com.dsc.easyflowDotNet.kernelBasePage.EFMIMJBasepage
	{
		...
		protected override void MJFunction()
		{
			...
			switch(tMJ)//欄位ID命名要規則化
			{
				...
				{
					bSelfResult = getMJResultByDB("OpenMIMJ01","20",1,FinalList, @"192.168.10.22", "price","MasterPage_MasterPageContent_opentype_txt2=type_name");
					if(bSelfResult)
					{
						if (tAllInputValues["MasterPage_MasterPageContent_openitem_txt"] != null && tAllInputValues["MasterPage_MasterPageContent_openitem_txt"].ToString().Length > 0){
							...
							aryParamger[1]=tAllInputValues["MasterPage_MasterPageContent_opentype_txt"].ToString();
							//自訂MsgCode
