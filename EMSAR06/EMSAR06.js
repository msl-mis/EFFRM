...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar06003 = $('#MasterPage_MasterPageContent_emsar06003_txt');
	if(temsar06003.length>0){
		var temsar06003Value = $('#MasterPage_MasterPageContent_emsar06003_txt').val().trim();
		if (temsar06003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR06', 'emsar06003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR06.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar06004_txt").value;         
    }
}