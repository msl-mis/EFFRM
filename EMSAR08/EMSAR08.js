...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar08003 = $('#MasterPage_MasterPageContent_emsar08003_txt');
	if(temsar08003.length>0){
		var temsar08003Value = $('#MasterPage_MasterPageContent_emsar08003_txt').val().trim();
		if (temsar08003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR08', 'emsar08003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR08.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    if(divContext != null)
    {
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar08004_txt").value;         
    }
}