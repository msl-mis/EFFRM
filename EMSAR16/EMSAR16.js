...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar16003 = $('#MasterPage_MasterPageContent_emsar16003_txt');
	if(temsar16003.length>0){
		var temsar16003Value = $('#MasterPage_MasterPageContent_emsar16003_txt').val().trim();
		if (temsar16003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR16', 'emsar16003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR16.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar16004_txt").value;
