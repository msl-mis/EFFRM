	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMWPDCN01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMWPDCN01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+='CN假日生產安排表-第'+
$('#MasterPage_MasterPageContent_txtwek_txt').val()+'周';
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMWPDCN01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
