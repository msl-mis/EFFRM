...
function CustomerSaveCheck_Head(tStatus) {
	...
	if (tStatus == "CREATE") {
		...
		//		{
		//			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money01_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		//		}
		//		else
		//		{
		...
		//		{
		//			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money02_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		//		}
		//		else
		//		{
		...
		//		{
		//			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money03_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
		//		}
		//		else
		//		{
	...
	if (texpdate.length > 0) {
		...
		if (texpdateValue.length == 0) {
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'expdate', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if (titem01.length > 0) {
		...
		if (titem01Value.length == 0) {
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'item01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if (tmoney01.length > 0) {
		...
		if (tmoney01Value.length == 0) {
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if (tmoney01.length > 0) {
		...
		if (tmoney01Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney01Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else {
	...
	if (tmoney02.length > 0) {
		...
		if (tmoney02Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney02Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else {
	...
	if (tmoney03.length > 0) {
		...
		if (tmoney03Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney03Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else {
	...
	if (ttotomoney.length > 0) {
		...
		if (ttotomoneyValue.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(ttotomoneyValue)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'totomoney', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else {
				ttotomoneyValue = ttotomoneyValue.replace(/\,/g, '');
				$('#MasterPage_MasterPageContent_totomoney_txt').val(Math.round(parseFloat(ttotomoneyValue) * Math.pow(10, 2)) / Math.pow(10, 2));
			}
		}
	}
...
function DraftSaveCheck() {
	...
	var tmoney01 = $('#MasterPage_MasterPageContent_money01_txt');
	if (tmoney01.length > 0) {
		var tmoney01Value = $('#MasterPage_MasterPageContent_money01_txt').val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;
		if (tmoney01Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney01Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
	...
	var tmoney02 = $('#MasterPage_MasterPageContent_money02_txt');
	if (tmoney02.length > 0) {
		var tmoney02Value = $('#MasterPage_MasterPageContent_money02_txt').val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (tmoney02Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney02Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
	...
	var tmoney03 = $('#MasterPage_MasterPageContent_money03_txt');
	if (tmoney03.length > 0) {
		var tmoney03Value = $('#MasterPage_MasterPageContent_money03_txt').val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (tmoney03Value.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney03Value)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'money03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
	...
	var ttotomoney = $('#MasterPage_MasterPageContent_totomoney_txt');
	if (ttotomoney.length > 0) {
		var ttotomoneyValue = $('#MasterPage_MasterPageContent_totomoney_txt').val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (ttotomoneyValue.length > 0) {
			if (!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(ttotomoneyValue)) {
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMEXPVL01', 'totomoney', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
	...
	if ($('#MasterPage_MasterPageContent_' + pNumberContrlID + '_txt').length > 0 && $('#MasterPage_MasterPageContent_' + pNumberContrlID + '_txt').is(':visible')) {
		...
		if (tControlNumValue.length > 0) {
			if (isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMEXPVL01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMEXPVL01.ajaxGetMoneyWord(tControlNumValue).value;
...
function domath_totomoney() {
	try {
		if ($("#MasterPage_MasterPageContent_money01_txt").length == 0)
			return;
		var money01 = $("#MasterPage_MasterPageContent_money01_txt").val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (isNaN(money01)) {
		...
		if ($("#MasterPage_MasterPageContent_money02_txt").length == 0)
			return;
		var money02 = $("#MasterPage_MasterPageContent_money02_txt").val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (isNaN(money02)) {
			$("#MasterPage_MasterPageContent_totomoney_txt").val("");
			return;
		}
		...
		if ($("#MasterPage_MasterPageContent_money03_txt").length == 0)
			return;
		var money03 = $("#MasterPage_MasterPageContent_money03_txt").val().trim().replace(/\,/g, ''); // Peggy 先移除千份位;;;
		if (isNaN(money03)) {
			$("#MasterPage_MasterPageContent_totomoney_txt").val("");
			return;
		}
		...
		document.getElementById("MasterPage_MasterPageContent_totomoney_txt").value = Math.round(parseFloat(finalvalue) * Math.pow(10, 2)) / Math.pow(10, 2);
		...
		document.getElementById("MasterPage_MasterPageContent_totomoney_txt").value = OEMFormat(parseFloat(finalvalue).toFixed(2));
	} catch (err) {
		$("#MasterPage_MasterPageContent_totomoney_txt").val("");
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID) {
	...
	if (window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())) {
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMEXPVL01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
