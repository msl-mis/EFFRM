...
function CustomerSaveCheck_Head(tStatus)
{
	...
	//填表及簽核都要驗證
	//不允許空白驗證
	var tmslpartno = $('#MasterPage_MasterPageContent_mslpartno_txt');
	if(tmslpartno.length>0){
		var tmslpartnoValue = $('#MasterPage_MasterPageContent_mslpartno_txt').val().trim();
		if (tmslpartnoValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMREPMVE01', 'mslpartno', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}

	//不允許空白驗證
	var topensetion = $('#MasterPage_MasterPageContent_opensetion_txt');
	if(topensetion.length>0){
		var topensetionValue = $('#MasterPage_MasterPageContent_opensetion_txt').val().trim();
		if (topensetionValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMREPMVE01', 'opensetion', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMREPMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMREPMVE01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf='';
	tSubjectSelf+='MVE樣板評定報告-'+
$('#MasterPage_MasterPageContent_mslpartno_txt').val();
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMREPMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
