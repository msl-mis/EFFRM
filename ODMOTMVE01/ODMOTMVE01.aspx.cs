...
namespace tw.com.dsc.easyflowDotNet.forms
{
	...
	/// ODMOTMVE01 的摘要描述。
	/// </summary>
	public partial class ODMOTMVE01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMOTMVE01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMOTMVE01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMOTMVE01", "HeadTabStrip01", tLanguageType);
			...
			this.txtdept.Title = MultiLanguage.GetComment("FD", "ODMOTMVE01", "txtdept", tLanguageType);
			this.date.Title = MultiLanguage.GetComment("FD", "ODMOTMVE01", "date", tLanguageType);
			this.txtmain.Title = MultiLanguage.GetComment("FD", "ODMOTMVE01", "txtmain", tLanguageType);
			this.txtuser.Title = MultiLanguage.GetComment("FD", "ODMOTMVE01", "txtuser", tLanguageType);
		...
		{
			// 作業代號
			this.TaskId = "ODMOTMVE01";
			// 表單代號
			this.formID = "ODMOTMVE01";
...
this.kind_ctrolRadio1.Attributes.Add("onclick", "document.getElementById('MasterPage_MasterPageContent_kind_txt').value = '1';");
		...
		{
			defalutHash.Add("odmotmve01001", this.formID);
			defalutHash.Add("odmotmve01002", this.SheetNo);
...
order by resdd003 desc";
		...
		{
			pReport = "ODMOTMVE01";
			pReportID = "ODMOTMVE01_02";//憑證式
			pWhere = "AND (odmotmve01001='" + this.formID + "') AND (odmotmve01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMOTMVE01";
