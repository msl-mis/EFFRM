...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar03003 = $('#MasterPage_MasterPageContent_emsar03003_txt');
	if(temsar03003.length>0){
		var temsar03003Value = $('#MasterPage_MasterPageContent_emsar03003_txt').val().trim();
		if (temsar03003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR03', 'emsar03003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR03.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar03004_txt").value;
