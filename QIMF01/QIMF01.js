...
var gField_companyid = "qimf01003";//公司別代號的id
//單頭自訂驗證
...
//2013/02/04:3.5.1.48:hiro:S00-20120626008:雙檔表單簽核鈕(發單、准、同意、會辦)增加單身驗證功能↑
{
	...
	//填表及簽核都要驗證
	//不允許空白驗證
	var tqimf01005 = $('#MasterPage_MasterPageContent_qimf01005_txt');
	if(tqimf01005.length>0){
		var tqimf01005Value = $('#MasterPage_MasterPageContent_qimf01005_txt').val().trim();
		if (tqimf01005Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'QIMF01', 'qimf01005', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}	

    //^_^20190828 edit by 06475 lijyun 將數值三位撇取消↓
	var qimf01010 = $('#MasterPage_MasterPageContent_qimf01010_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf01010_txt').val(parseFloat(qimf01010));
	var qimf01017 = $('#MasterPage_MasterPageContent_qimf01017_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf01017_txt').val(parseFloat(qimf01017));
	var qimf01018 = $('#MasterPage_MasterPageContent_qimf01018_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf01018_txt').val(parseFloat(qimf01018));
	var qimf01019 = $('#MasterPage_MasterPageContent_qimf01019_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf01019_txt').val(parseFloat(qimf01019));
	var qimf01020 = $('#MasterPage_MasterPageContent_qimf01020_txt').val().replace(/\,/g, '');
	$('#MasterPage_MasterPageContent_qimf01020_txt').val(parseFloat(qimf01020));
    //^_^20190828 edit by 06475 lijyun 將數值三位撇取消↑
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.QIMF01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.QIMF01.ajaxGetMoneyWord(tControlNumValue).value;
...
function SetCustomSubject()
{
	...
	var tSubjectSelf = '';
	var tTitle = getI18NForSpecial('FD', 'QIMF01', 'HeadTabStrip01', '../../../src/_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢查記錄表
    //^_^ 20191205 edit by 06475 lijyun DSCBV0972A_38_0001↓
	tSubjectTxt += $("#MasterPage_MasterPageContent_qimf01005_txt").val() + "(" + $("#MasterPage_MasterPageContent_qimf01009_txt2hdn").val() + ")";
    //^_^ 20191205 edit by 06475 lijyun DSCBV0972A_38_0001↑
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.QIMF01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
...
function qimf01006_onblur() {
    var UniID = document.getElementById("MasterPage_universalID").value;
    var qimf01024 = document.getElementById("MasterPage_MasterPageContent_qimf01024_txt"); //圖面一
    var qimf01025 = document.getElementById("MasterPage_MasterPageContent_qimf01025_txt"); //圖面二
    var qimf01026 = document.getElementById("MasterPage_MasterPageContent_qimf01026_txt"); //圖面三
    var qimf01006 = document.getElementById("MasterPage_MasterPageContent_qimf01006_txt"); //品號
    var qimf01015 = document.getElementById("MasterPage_MasterPageContent_qimf01015_txt"); //品號
    var companyid = document.getElementById("MasterPage_MasterPageContent_" + gField_companyid + "_ddl"); //品號

    //圖面一
    if (qimf01024 != null && qimf01024.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF01.LoadQIMF01IMG(qimf01024.value, "Drawing1", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01024").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf01024").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01024").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf01024").style.display = "none";
    ...
    //圖面二
    if (qimf01025 != null && qimf01025.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF01.LoadQIMF01IMG(qimf01025.value, "Drawing2", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01025").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf01025").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01025").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf01025").style.display = "none";
    ...
    //圖面三
    if (qimf01026 != null && qimf01026.value != "") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF01.LoadQIMF01IMG(qimf01026.value, "Drawing3", UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01026").style.display = "";
                document.getElementById("MasterPage_MasterPageContent_imgqimf01026").src = aryRet[1];
            }
            else {
                document.getElementById("MasterPage_MasterPageContent_imgqimf01026").style.display = "none";
                alert(aryRet[1]);
            }
        } else {
            document.getElementById("MasterPage_MasterPageContent_imgqimf01026").style.display = "none";
    ...
    //抽樣計畫表
    if (companyid != null && companyid.value != "" && qimf01006 != null && qimf01006.value != ""
        && qimf01015 != null && qimf01015.value!="") {
        var tRet = tw.com.dsc.easyflowDotNet.forms.QIMF01.LoadSamplingPlanIMG(companyid.value, qimf01006.value, qimf01015.value, UniID).value;
        if (tRet != "") {
            var aryRet = tRet.split('§');
            if (aryRet[0] == "Y" && aryRet[1] != "") {
...
function qimf01005_onblur() {
    document.getElementById("MasterPage_MasterPageContent_qimf01006_txt").onchange();
    document.getElementById("MasterPage_MasterPageContent_qimf01006_txt").onblur();
}
...
function LoadJSONObject(tStatus) {
    var qimf01001 = document.getElementById("MasterPage_MasterPageContent_qimf01001_txt");//表單代號
    var qimf01002 = document.getElementById("MasterPage_MasterPageContent_qimf01002_txt");//表單單號
    var qimf01006 = document.getElementById("MasterPage_MasterPageContent_qimf01006_txt");//產品品號
    var qimf01015 = document.getElementById("MasterPage_MasterPageContent_qimf01015_txt");//檢驗版本
    var qimf01017 = document.getElementById("MasterPage_MasterPageContent_qimf01017_txt");//抽樣數
    var companyid = document.getElementById("MasterPage_MasterPageContent_" + gField_companyid + "_ddl"); //品號
    
    if (qimf01006 != null && qimf01006.value != "" && qimf01015 != null && qimf01015.value != "") {
        //先判斷是否有未完成的檢驗資料，取得檢驗資料
        obj_return = tw.com.dsc.easyflowDotNet.forms.QIMF01.ajaxLoadJSON([qimf01006.value, qimf01015.value, qimf01017.value, companyid.value, qimf01001.value, qimf01002.value]);
        if (obj_return.value != "") {
...
function DarwingCheckData(tStatus) {
    var tqimf01d003 = getI18NForSpecial('FD', 'QIMF01_d01', 'NO', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗項目序號
    var tqimf01d004 = getI18NForSpecial('FD', 'QIMF01_d01', 'qimf01d003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗項目
    var tqimf01d005 = getI18NForSpecial('FD', 'QIMF01_d01', 'qimf01d004', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗說明
	var tqimf01d006 = getI18NForSpecial('FD', 'QIMF01_d01', 'qimf01d006', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//檢驗結果
	var tqimf01d010 = getI18NForSpecial('FD', 'QIMF01_d01', 'qimf01d010', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//Min 
	var tqimf01d011 = getI18NForSpecial('FD', 'QIMF01_d01', 'qimf01d011', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx');//Max

    var thint_img = "../../_Common/AppUtil/Themes/images/Form/imgImportant.gif";

    var qimf01006 = document.getElementById("MasterPage_MasterPageContent_qimf01006_txt");//料號
    var qimf01015 = document.getElementById("MasterPage_MasterPageContent_qimf01015_txt");//檢驗版本
    var qimf01017 = document.getElementById("MasterPage_MasterPageContent_qimf01017_txt");//抽樣數
    var icount = parseInt(qimf01017.value);
    ...
    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf01d003).css("height", "63px"); row.append(cell);
    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").addClass("OEMTableWidth").text(tqimf01d004); row.append(cell);

    cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf01d005).addClass("OEMTableWidth").addClass("OEMTableWidth");
	row.append(cell);

	//^_^ 20201219 edit by 13768 yating DSCBV0972A_38_0004 ↓
	cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf01d010).addClass("OEMTableWidth").addClass("OEMTableWidth");
	row.append(cell); 

	cell = $("<td>").attr("rowspan", "2").addClass("OEMFormFont").text(tqimf01d011).addClass("OEMTableWidth").addClass("OEMTableWidth");
	row.append(cell); 
	//^_^ 20201219 edit by 13768 yating DSCBV0972A_38_0004 ↑

    cell = $("<td>").attr("colspan", icount).addClass("OEMFormFont").text(tqimf01d006); row.append(cell);
    row.append(cell);
    table.append(row);
    row = $("<tr>").addClass("GridviewScrollHeader");
