...
function CustomerSaveCheck(tStatus)
{
	...
	//不允許空白驗證
	var temsar15003 = $('#MasterPage_MasterPageContent_emsar15003_txt');
	if(temsar15003.length>0){
		var temsar15003Value = $('#MasterPage_MasterPageContent_emsar15003_txt').val().trim();
		if (temsar15003Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'EMSAR15', 'emsar15003', '../../_Common/PlatFormUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
{ 
   ...
   var tRetVal = tw.com.dsc.easyflowDotNet.forms.EMSAR15.GetDetialRowCount(universalID); 
   if(tRetVal.value == '0') 
   { 
...
function emsOnload()
{
    ...
    {        
         ...
         divContext.innerHTML = document.getElementById("MasterPage_MasterPageContent_emsar15004_txt").value;
