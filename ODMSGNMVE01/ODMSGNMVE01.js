...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if (tStatus == "CREATE")
	{
		...
		//Radio Button 驗証
			...
			{
				tErr += getI18NForSpecial('FD', 'ODMSGNMVE01', 'cate_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
			}
	...
	if(ttxtmain.length>0){
		...
		if (ttxtmainValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMSGNMVE01', 'txtmain', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(tStatus=="CREATE" || tStatus=="DISPLAY" || tStatus==""){
		if($("#MasterPage_MasterPageContent_cate_ctrolRadio3").length>0){
			if($("#MasterPage_MasterPageContent_cate_ctrolRadio3")[0].checked){
				$("#MasterPage_MasterPageContent_other").show();
			}
			else{
				$("#MasterPage_MasterPageContent_other").hide();
			}
		}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMSGNMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMSGNMVE01.ajaxGetMoneyWord(tControlNumValue).value;
...
function chkTriggerFieldNull_Head()
{
	...
	if($("#MasterPage_MasterPageContent_cate_ctrolRadio3").length>0){
		if($("#MasterPage_MasterPageContent_cate_ctrolRadio3")[0].checked){
			if($("#MasterPage_MasterPageContent_other_txt").val().trim().length==0){
				tErr+='[other-'+getI18NForSpecial('FD', 'ODMSGNMVE01', 'other', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#MasterPage_MasterPageContent_other_txt").val('');
		}
	}
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMSGNMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
