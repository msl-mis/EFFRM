...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if(ttotalmoney.length>0){
		...
		if (ttotalmoneyValue.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(ttotalmoneyValue)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'ODMCONMVE01', 'totalmoney', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(ttxtmain.length>0){
		...
		if (ttxtmainValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMCONMVE01', 'txtmain', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMCONMVE01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMCONMVE01.ajaxGetMoneyWord(tControlNumValue).value;
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMCONMVE01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
