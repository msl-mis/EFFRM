...
namespace tw.com.dsc.easyflowDotNet.forms
{
	...
	/// ODMCONMVE01 的摘要描述。
	/// </summary>
	public partial class ODMCONMVE01 : tw.com.dsc.easyflowDotNet.kernelBasePage.EFBasePage
	{
		//edit by teppy 2011/02/08 Start
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			AjaxPro.Utility.RegisterTypeForAjax(typeof(ODMCONMVE01));
			UserInfoClass tClass = (UserInfoClass)Session["UserInfo"];
			string tLanguageType = tClass.Language;

			//多國語系
			this.Title= MultiLanguage.GetComment("FD","ODMCONMVE01","lblTitle",this.UserInfo.Language);
			string tHeadTabStrip01Text = MultiLanguage.GetComment("FD", "ODMCONMVE01", "HeadTabStrip01", tLanguageType);
			...
			this.txtdept.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "txtdept", tLanguageType);
			this.date.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "date", tLanguageType);
			this.txtmain.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "txtmain", tLanguageType);
			this.txtuser.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "txtuser", tLanguageType);
			this.totalmoney.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "totalmoney", tLanguageType);
			this.superid.Title = MultiLanguage.GetComment("FD", "ODMCONMVE01", "superid", tLanguageType);
		...
		{
			// 作業代號
			this.TaskId = "ODMCONMVE01";
			// 表單代號
			this.formID = "ODMCONMVE01";
...
where resfc001=@resfc001 and resfc002=@resfc002 and resfc003=@resfc003 and ISNULL(resfc004,'')<>''";
		...
		protected override void settingClientFunction()
		{
			...
			//Peggy Start
            #region 金額三位一撇
            TextBox2[] arrayTextBox = { totalmoney.TxtInput };
		...
		{
			defalutHash.Add("odmconmve01001", this.formID);
			defalutHash.Add("odmconmve01002", this.SheetNo);
...
order by resdd003 desc";
		...
		protected override void ReGetCondValue(object[,] pAryCondValue)
		{
			...
			tDbl=0;
			try{
				tDbl = double.Parse(this.totalmoney.Value.Trim());
			}
			catch(Exception e){
				tDbl=0;
			}
			pAryCondValue[0, 0] = "totalmoney";
		...
		{
			pReport = "ODMCONMVE01";
			pReportID = "ODMCONMVE01_02";//憑證式
			pWhere = "AND (odmconmve01001='" + this.formID + "') AND (odmconmve01002='" + this.SheetNo + "')";
			pReportDirectory = "ODMCONMVE01";
