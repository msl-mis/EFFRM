...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if (tStatus == "CREATE")
	{
		//填表時要驗證
		//Radio Button 驗証
			if ($('#MasterPage_MasterPageContent_cate_txt').length>0 && document.getElementById('MasterPage_MasterPageContent_cate_txt').value == '')
			{
				tErr += getI18NForSpecial('FD', 'ODMSGNCN01', 'cate_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
			}
	...
	if(ttxtmain.length>0){
		...
		if (ttxtmainValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'ODMSGNCN01', 'txtmain', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
...
function InitTriggerMust(tStatus){
	if(tStatus=="CREATE" || tStatus=="DISPLAY" || tStatus==""){
		if($("#MasterPage_MasterPageContent_cate_ctrolRadio3").length>0){
			if($("#MasterPage_MasterPageContent_cate_ctrolRadio3")[0].checked){
				$("#MasterPage_MasterPageContent_text2").show();
			}
			else{
				$("#MasterPage_MasterPageContent_text2").hide();
			}
		}
	...
	if($('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').length>0 && $('#MasterPage_MasterPageContent_'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.forms.ODMSGNCN01.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.forms.ODMSGNCN01.ajaxGetMoneyWord(tControlNumValue).value;
...
function chkTriggerFieldNull_Head()
{
	...
	if($("#MasterPage_MasterPageContent_cate_ctrolRadio3").length>0){
		if($("#MasterPage_MasterPageContent_cate_ctrolRadio3")[0].checked){
			if($("#MasterPage_MasterPageContent_text2_txt").val().trim().length==0){
				tErr+='[text2-'+getI18NForSpecial('FD', 'ODMSGNCN01', 'text2', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#MasterPage_MasterPageContent_text2_txt").val('');
		}
	}
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.forms.ODMSGNCN01.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
